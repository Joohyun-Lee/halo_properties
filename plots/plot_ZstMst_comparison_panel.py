...
for iplot, out_nb in enumerate(out_nbs):
    ...
    for assoc_mthd, ll, r200 in zip(assoc_mthds, lls, r200s):
        ...
        out_file = os.path.join(out_path, f'MZstMs_{out_nb:d}_{assoc_mthd:s}_{ll:.2f}_{r200:.1f}')
        ...
        if overwrite or not exists:
            ...
            bins, med = xy_stat(Mst, Zst, mbins, mthd=stat_mthd)

            with h5py.File(out_file, 'w') as dest:
                dest.create_dataset("stellar mass", data = bins, dtype='f4')
                dest.create_dataset("Zst", data = med, dtype='f8')
        ...
        line = plot_zstmst(fig, ax, bins, med/0.02, linewidth=3, elinewidth=2, capsize=5, xscale='log', yscale='log')
    ...
    # obs_lines, obs_labels = plot_zstmst_constraints(ax, redshift)
    obs_labels = []
    obs_lines = []
    dustier_lines ,dustier_labels = plot_dustier_zstmst(ax, redshift, color='k')
...
fig_name = f'./figs/ZstMst_comparison_panel'

...
from plot_functions.UV.extinction import plot_ext, plot_dustier_ext, make_ext

import os
import h5py


def load_mags(sim_name, out_nb, dset, ext_keys):
    out, assoc_out, analy_out, suffix = gen_paths(sim_name, out_nb, dset)

    # print(analy_out)

    # try:
    datas = gather_h5py_files(analy_out, keys=ext_keys)
    # except OSError as e:
    # print(f'OSError : {out_nb:d} ll={ll:f}, {rtwo_fact:f}xr200, association: {assoc_mthd:s}, {fesc_key:s}, xkey={xkey:s}')
    # print(e)

    return [datas[ext_key][()] for ext_key in ext_keys]
...
mag_ext_keys = ["mag_" + k for k in ext_keys if "no_dust" not in k]
...
for iplot, out_nb in enumerate(out_nbs):
    ...
    for assoc_mthd, ll, r200, mp, clean in zip(assoc_mthds, lls, r200s, mps, cleans):
        for ext_key, mag_ext_key in zip(ext_keys, mag_ext_keys):
            ...
            if overwrite or not exists:
                ...
                bins, ext = make_ext(mags[1], mags[0], mag_bins)
            ...
            # print(list(zip(bins, ext, err)))
            # print()
            # ext[ext>0]= ext[ext>0]+0.25
            line = plot_ext(
                fig,
                ax,
                bins,  # + 2.5 * np.log10(1.0 / 0.8),
                ext,
                linewidth=3,
                elinewidth=1,
                capsize=3,
    ...
    dust_line, dust_label = plot_dustier_ext(ax, redshift, c="k")
    # obs_lines, obs_labels = plot_constraints(fig, ax, redshift, prec_zed=0.5)
    obs_lines = [] + dust_line
    obs_labels = [] + dust_label
...
fig_name = f"./figs/ext_comparison_panel_comp"

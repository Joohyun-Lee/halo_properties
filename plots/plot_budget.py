...
def load_data(sim_name, out_nb, dset, fesc_type="gas", xkey="mass"):
    ...
    # print(
    #     f"LOADING : {out_nb:d} ll={ll:f}, {rtwo_fact:f}xr200, association: {assoc_mthd:s}"
    # )
...
# plot_bins = np.linspace(-25, -5, 6)
plot_bins = np.sort(np.asarray([0, -13, -15, -17, -19, -20, -21, -25]))
# plot_bin_labels = ["%.1e-%.1e"%(mm1,mp1) for mm1,mp1 in zip(plot_bins[:-1], plot_bins[1:])]
plot_bin_labels = [
    "[%d,%d]" % (mm1, mp1) for mm1, mp1 in zip(plot_bins[:-1], plot_bins[1:])
]
...
for i_out, out_nb in enumerate(out_nbs):
    ...
    out_file = os.path.join(
        out_path,
        f"budget_{redshift:.1f}_{stat_mthd:s}_{assoc_mthd:s}_{ll:.2f}_{r200:.1f}_{fesc_type:s}",
    )
    ...
    if overwrite or not exists or no_key:
        ...
        lesc, datas = load_data(
            "CoDaIII", out_nb, dset, fesc_type=fesc_type, xkey=x_plot
        )
        ...
        with h5py.File(out_file, "a") as dest:
            ...
            if np.any(xs < 0):
                ...
                xbins = np.logspace(
                    np.log10(xmin),
                    np.log10(xmax),
                    int(np.abs(np.log10(xmax) - np.log10(xmin))) * 4,
                )

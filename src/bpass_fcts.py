...
def get_mag_tab_BPASSV221_betas(file):
    ...
    l_metal_bins = len(metal_bins)

    xis = np.vstack(reading[4 : 4 + l_metal_bins])
    mags = np.vstack(reading[4 + l_metal_bins : 4 + 2 * l_metal_bins])
    contbetalow = np.vstack(reading[4 + 2 * l_metal_bins : 4 + 3 * l_metal_bins])
    contbetahigh = np.vstack(reading[4 + 3 * l_metal_bins : 4 + 4 * l_metal_bins])
    beta = np.vstack(reading[4 + 4 * l_metal_bins : 4 + 5 * l_metal_bins])
...
def get_star_mags_metals(star_age, star_metal, interp_fct):
    ...
    return np.asarray(
        [interp_fct(np.log10(age), np.log10(Z))[0] for age, Z in zip(ages, Zs)]
    )


def get_xis_interp_fct(xis_tab, Agebins, Zbins):
    return interp2d(
        np.log10(Agebins), np.log10(Zbins), np.log10(xis_tab), kind="linear"
    )
    ...
    if coef_att.Kappa912 > 0.0:
        betas = np.log10(
            np.sum(
                Mst
                * high_flux
                * np.exp(-tau_dust912 * coef_att.Kappa2500 / coef_att.Kappa912)
            )
            / np.sum(
                Mst
                * low_flux
                * np.exp(-tau_dust912 * coef_att.Kappa1500 / coef_att.Kappa912)
            )
        ) / np.log10(2500.0 / 1500.0)
    else:
        betas = np.log10(np.sum(Mst * high_flux) / np.sum(Mst * low_flux)) / np.log10(
            2500.0 / 1500.0
        )
...
def comp_betas_indv(Mst, high_flux, low_flux, tau_dust912, coef_att):
    ...
    if coef_att.Kappa912 > 0.0:
        ...
        betas = np.log10((Mst * high_flux) / (Mst * low_flux)) / np.log10(
            2500.0 / 1500.0
        )

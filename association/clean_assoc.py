...
from halo_properties.utils.utils import (
    merge_arrays_rank0,
    sum_arrays_to_rank0,
    scatter_arrays_rank0,
)


def clean_assoc(
    out_nb, rtwo_fact=1.0, assoc_mthd="stellar_peak", ll=0.2, overwrite=False, mp=True
):
    if sixdigits:
    ...
    dset = dataset(ll=ll, r200=rtwo_fact, assoc_mthd=assoc_mthd, mp=mp, clean=False)
    ...
    # print(assoc_out, analy_out, out_file)

    if os.path.isfile(out_file) and not overwrite:
        if rank == 0:
            print(f"File exists ({out_file:s}) and overwrite is False")
        return
    ...
    # enable only ready list of required fields
    sub_halo_tab, halo_star_ids, sub_halo_tot_star_nb = read_assoc(
        out_nb, sim_name, dset, 512
    )

    # only perform on one node and then share?
    if rank == 0:
        uniques, u_count = np.unique(halo_star_ids, return_counts=True)
        mult_uniques = uniques[u_count > 1][:]
        mult_u_counts = u_count[u_count > 1][:]
    else:
        mult_uniques = None
        mult_u_counts = None
    ...
    loc_mult_uniques = scatter_arrays_rank0(comm, mult_uniques, dtype="i8")
    loc_mult_u_counts = scatter_arrays_rank0(comm, mult_u_counts, dtype="i8")
    ...
    # del mult_uniques, uniques, u_count
    ...
    stellar_id_args_to_del = np.zeros(
        np.sum(loc_mult_u_counts) - len(loc_mult_u_counts), dtype=np.int32
    )
    ...
    for itgt, tgt_unique in enumerate(loc_mult_uniques):
        ...
        halo_mstar_red[not_closest_u_halo] = (
            halo_mstar_red[not_closest_u_halo] - star["mass"][0]
        )
        ...
        for h_arg in not_closest_u_halo:
            ...
            stellar_id_args_to_del[i_id_to_del] = (
                star_u_args[(star_u_args < hi) * (star_u_args >= lo)]
            )[0]
            i_id_to_del += 1
    ...
    stellar_id_args_to_del = merge_arrays_rank0(
        comm, np.int32(stellar_id_args_to_del), dtype=np.int32
    )
    ...
    if rank == 0:
        ...
        with h5py.File((out_file), "w") as out_halos:
            out_halos.create_dataset(
                "ID",
                data=np.int64(sub_halo_tab["ids"][()]),
                dtype=np.int64,
                compression="lzf",
            )
            # out_halos.create_dataset(
            ...
            out_halos.create_dataset(
                "mass",
                data=sub_halo_tab["mass"][()],
                dtype=np.float32,
                compression="lzf",
            )
            out_halos.create_dataset(
                "coords",
                data=np.asarray(sub_halo_tab[["x", "y", "z"]].tolist()),
                dtype=np.float64,
                compression="lzf",
            )
            out_halos.create_dataset(
